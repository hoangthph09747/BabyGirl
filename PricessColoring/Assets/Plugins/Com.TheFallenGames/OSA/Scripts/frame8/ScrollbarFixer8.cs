		...
		IEnumerator SlowUpdate()
		{
            ...
            while (true)
            {
                ...
                if (_ScrollViewRT && (scrollRect && scrollRect.content || externalScrollRectProxy != null && externalScrollRectProxy.IsInitialized))
                {
					...
					{
						sizeUnclamped = size = viewportSize / contentSize;
						...
						if (outsideAmtRaw > 0d)
						{
							size -= size * outsideAmtRaw / viewportSize;
							minToUse = minCompressedSize;
						}
						else
							minToUse = minSize;

						size = System.Math.Max(minToUse, System.Math.Min(maxSize, size));
					}
