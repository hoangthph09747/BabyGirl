...
public class CameraDrag : MonoBehaviour
{
    ...
    [Range(0, 30)] public float speed = 10f; //lerp speed
    ...
    public bool isCameraDraging; //whether the camera is draging or not
    ...
    private void TouchRelease()
    {
        ...
        currentTouchPosition.x =
            currentTouchPosition.x + (currentTouchPosition.x - previousTouchPosition.x) * releaseFactor;
        currentTouchPosition.y =
            currentTouchPosition.y + (currentTouchPosition.y - previousTouchPosition.y) * releaseFactor;
    ...
    private void TouchBegan(Vector3 screenTouchPosition)
    {
        ...
        currentTouchPosition =
            (targetCamera.ScreenToWorldPoint(screenTouchPosition) * (1.0f / targetCamera.orthographicSize)) *
    ...
    private void LerpToTouchPosition()
    {
        ...
        xDistance = currentTouchPosition.x -
                    firstTouchPosition.x; //the x distance between the first touch and the current touch
        yDistance = currentTouchPosition.y -
                    firstTouchPosition.y; //the y distance between the first touch and the current touch

        xMirrorPosition =
            firstTouchPosition.x -
            (offset.x + xDistance); //the mirror or the opposite x position of the (offset.x + xDistance)
        yMirrorPosition =
            firstTouchPosition.y -
            (offset.y + yDistance); //the mirror or the opposite y position of the (offset.y + yDistance)
