    ...
    public class CameraSizeHandler
    {
        ...
        static Dictionary<CameraOrientation, Quaternion> rotationByOrientation
            = new Dictionary<CameraOrientation, Quaternion>()
            {
                ...
                {CameraOrientation.CustomOrientation, Quaternion.Euler(0.0f, 0.0f, 0.0f)},
            };
        ...
        public void Init(Camera camera, CanvasController canvas)
        {
            this.camera = camera;
            this.canvas = canvas;
            // if (DrawPictureController.Instance.Aspect is >= 1.33f and <= 1.45f)
            //     cameraSize = 710;
        ...
        void SetupCameraViewPort()
        {
            ...
            else if (Orientation == CameraOrientation.CustomOrientation)
            {
        ...
        public void LateUpdate()
        {
            if (enabled)
            {
                ...
                {
                    newPosition = Tween.ChangeVector(easeTime, startPosition, cameraMoveVector, MovementTime,
                        MovementType);
                }
        ...
        {
            float camYTopLimit = canvas.transform.position.y + (canvas.Height / 2.0f) + CanvasMargin.y -
                                 camera.orthographicSize;
            float camYBottomLimit = canvas.transform.position.y - (canvas.Height / 2.0f) - CanvasMargin.y +
                                    camera.orthographicSize;
            ...
            cameraTargetPosition.x = Mathf.Clamp(cameraTargetPosition.x, Mathf.Min(camLeftLimit, camRightLimit),
                Mathf.Max(camLeftLimit, camRightLimit));
            cameraTargetPosition.y = Mathf.Clamp(cameraTargetPosition.y, Mathf.Min(camYBottomLimit, camYTopLimit),
                Mathf.Max(camYBottomLimit, camYTopLimit));
