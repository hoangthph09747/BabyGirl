...
Shader "Spaventacorvi/Holographic/Holo E - Bumped Specular Textured" {
    Properties {
        ...
        wvvxxv ("Background transp (Black)", Range(0, 1)) = 0
        ...
        vwwvvx ("Cubemap", Cube) = "_Skybox" {}
        wxvxxw ("Cubemap power (0-1)", Range(0, 1)) = 0.5
        wxxxvv ("Cubemap blur holo (0 - 5)", Range(0, 5)) = 0
        wxxxvw ("Cubemap blur tex (0 - 5)", Range(0, 5)) = 0
        wvwvww ("Specular mask", 2D) = "white" {}
        xvwvwx ("Holo map", 2D) = "bump" {}
        xvxwww ("Holo depth", Range(0, 1)) = 0.2
        wwvxvx ("Holo noise (0 - 0.5)", Range(0, 0.5)) = 0.2
        vwvxxv ("Normal map", 2D) = "bump" {}
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform float wvvxxv;
            uniform float vxxxwx;
            uniform sampler2D vwvxxv; uniform float4 vwvxxv_ST;
            ...
            uniform sampler2D wvwvww; uniform float4 wvwvww_ST;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 vwvxxv_var = UnpackNormal(tex2D(vwvxxv,TRANSFORM_TEX(i.uv0, vwvxxv)));
                float3 normalLocal = vwvxxv_var.rgb;
                ...
                float wxxxww = (vxvvvw_var.r*vxxxwx);
                float node_4713 = (1.0 - wxxxww);
                ...
                float3 vxwwww = ((vvxxww.rgb*(vwwxwx_var.rgb*vwxvww))+(wvxwxw*wwvxvx));
                float vwxwww = (1.0 - wvvxxv);
                float3 specularColor = (node_4713*lerp((_MainTex_var.rgb*vxwwww),vxwwww,vwxwww));
                ...
                float3 vxwvxx = lerp((vwwvxw*vwwxwx_var.rgb),dot((vwwvxw*vwwxwx_var.rgb),float3(0.3,0.59,0.11)),1.0);
                float3 diffuseColor = lerp(lerp((vxwvxx*_MainTex_var.rgb),vxwvxx,vwxwww),(_Color.rgb*_MainTex_var.rgb),wxxxww);
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
                float4 wvwvww_var = tex2D(wvwvww,TRANSFORM_TEX(i.uv0, wvwvww));
                float3 emissive = (texCUBElod(vwwvvx,float4(viewReflectDirection,((node_4713*wxxxvv)+(wxxxww*wxxxvw)))).rgb*wxvxxw*wvwvww_var.rgb);
