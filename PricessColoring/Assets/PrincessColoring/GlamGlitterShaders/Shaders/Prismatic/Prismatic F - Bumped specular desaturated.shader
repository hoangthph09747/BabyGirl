...
Shader "Spaventacorvi/Prismatic/Prismatic F - Bumped specular desaturated" {
    Properties {
        ...
        xxwxww ("Desat. Frsnl (0 - 1)", Range(0, 1)) = 0.7
        wxvxxw ("Desat. near (0 - 1)", Range(0, 1)) = 0.5
        vvxxww ("Desaturation far (0 - 1)", Range(0, 1)) = 0
        wxxxvv ("Fade distance", Float ) = 40
        vxwvxx ("Cubemap", Cube) = "_Skybox" {}
        xxwvxx ("Cubemap power (0-1)", Range(0, 1)) = 0
        wwxxvv ("Cubemap blur prism (0 - 5)", Range(0, 5)) = 0
        wxxxvw ("Cubemap blur tex (0 - 5)", Range(0, 5)) = 0
        wxxwxw ("Normal map", 2D) = "bump" {}
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform samplerCUBE vxwvxx;
            uniform float xxwvxx;
            ...
            uniform sampler2D wxxwxw; uniform float4 wxxwxw_ST;
            uniform sampler2D vxwwww; uniform float4 vxwwww_ST;
            uniform float wxxxww;
            uniform float wxvxxw;
            uniform float xxwxww;
            uniform float xvwvwx;
            uniform float wxxxvv;
            uniform float vvxxww;
            uniform float wwxxvv;
            uniform float wxxxvw;
            struct VertexInput {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 wxxwxw_var = UnpackNormal(tex2D(wxxwxw,TRANSFORM_TEX(i.uv0, wxxwxw)));
                float3 normalLocal = wxxwxw_var.rgb;
                ...
                float3 node_3784 = lerp(lerp(((vwwvvx.rgb*node_1034)+saturate(((vwxwww.rgb*wxxxww)-lerp(node_1034,dot(node_1034,float3(0.3,0.59,0.11)),1.0)))),lerp(vwwvxw.rgb,dot(vwwvxw.rgb,float3(0.3,0.59,0.11)),vvxxww),saturate((saturate((partZ/wxxxvv))*1.5+-0.5))),dot(lerp(((vwwvvx.rgb*node_1034)+saturate(((vwxwww.rgb*wxxxww)-lerp(node_1034,dot(node_1034,float3(0.3,0.59,0.11)),1.0)))),lerp(vwwvxw.rgb,dot(vwwvxw.rgb,float3(0.3,0.59,0.11)),vvxxww),saturate((saturate((partZ/wxxxvv))*1.5+-0.5))),float3(0.3,0.59,0.11)),saturate(((xxwxww*(1.0-max(0,dot(normalDirection, viewDirection))))+((1.0 - saturate(0.5*dot(i.normalDir,lightDirection)+0.5))*wxvxxw))));
                float4 vvxwwx_var = tex2D(vvxwwx,TRANSFORM_TEX(i.uv0, vvxwwx));
                float node_4363 = (vvxwwx_var.r*vvxxvv);
                float3 diffuseColor = lerp(lerp((wvwvww*node_3784),node_3784,(1.0 - wxxwww)),wvwvww,node_4363);
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
                float3 emissive = (texCUBElod(vxwvxx,float4(viewReflectDirection,(((1.0 - node_4363)*wwxxvv)+(node_4363*wxxxvw)))).rgb*xxwvxx);
